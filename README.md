...
传统的人物研究产出是文章、书评、传记——能读，不能用。Person-as-Skill 把人物蒸馏为可执行的 AI 原生格式：一个 SKILL.md 文件就是一个完整的认知操作系统。

**不是"请用 XX 的语气回答"。** 是让 AI 代理以该人物的认知框架处理问题。费曼会先查事实再下判断——所以他的 Skill 内置了 Agentic Protocol：先 WebSearch 获取最新信息，再用心智模型分析。芒格会先问"我会怎么死"——所以他的 Skill 有前置否定检查。

**每个 Skill 包含六个组件：**
- **身份卡**：人物背景与定位
- **核心心智模型**（5-10 个）：理论 + 案例 + 应用方式 + 局限
- **决策启发式**（8-19 条）：可操作的行为规则
- **表达 DNA**：句式、词汇、节奏、幽默方式
- **价值观与内在张力**：追求什么、拒绝什么、矛盾是什么
- **诚实边界**：框架在哪些领域会系统性失效

**诚实边界 = 认知精度标记。** 这不是免责声明。是告诉使用者"这个思维镜片的焦距在哪里模糊"——马斯克的思维模型在政治领域表现差，费曼在性别问题上有盲区，所有蒸馏基于公开信息截止某个时间点。精度标记让使用者知道什么时候该换另一副镜片。
